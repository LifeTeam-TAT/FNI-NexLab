	...
	public <T> List<LifeMedicalProposal> convertProposalDTOToProposal(T proposalDto) {
		...
		try {
			...
			snakeBiteDTO.getProposalInsuredPersonList().forEach(insuredPerson -> {
				...
				LifeMedicalCustomer customer = lifeProposalService.checkCustomerAvailabilityTemp(snakeBiteDTO.getCustomer());
				...
				lifeProposal.setOrganizationId(snakeBiteDTO.getOrganizationId());
